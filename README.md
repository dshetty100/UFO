...
- The filter needs to be manually cleared for every new search.
- Also, the filter takes search criteria only in lower cases. 

It would be advantageous to develop the webpage further by including the following:
- Having a drop down menu for the user to select the range of criteria for each filter.
- Having an "all clear" button that would clear the older entries for the users to enter new ones for the next search.
